<template>
  <div class="container mx-auto py-4">
    ...
    <div class="flex flex-col w-full">
      <!-- first content -->
      <div class="navbar gap-3 flex-wrap bg-base-300 rounded-box">
        <div class="dropdown dropdown-end">
          упорядочить по:&#x2000;
          <label tabindex="0" class="btn w-24 btn-secondary rounded-btn">{{ btnToggle ? "Городам" : "Профилям" }}</label>
        ...
        <div>
          сортировать по:&#x2000;
          <button class="btn btn-secondary w-28" @click="sortList">
            {{ sortToggle ? "убыванию" : "возрастанию" }}
          </button>
        </div>

        <input v-model="searchValue" @input="searchElements()" type="text" placeholder="Поиск по названию"
            class="input input-bordered input-info w-full max-w-xs placeholder:italic" />
      </div>
      <div class="divider"></div>
      <!-- second content -->
      <div class="grid h-60 card bg-base-300 rounded-box place-items-center">
        <div class="tabs">
          <button @click="getAllData" class="tab tab-lifted bg-accent" :class="{
            'tab-active': counter === -1,
          }">
            Все Университеты
          </button>
          <button v-for="(city, index) in getCurrentTabs" class="tab tab-lifted bg-accent" :class="{
    ...
    <div id="cards" class="mt-4 flex flex-wrap gap-4 md:gap-2 justify-center">
      <div id="singleCard" v-for="(vuz, index) in pagCards"
        class="card card-compact w-64 2xl:w-72  bg-base-100 shadow-xl mt-4 sm:mt-0" :key="vuz">
        <figure>
        ...
        <div class="card-body justify-between">
          ...
          <div class="card-actions justify-end mt-4">
            <button class="btn btn-primary">Подробная информация</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="join flex flex-wrap justify-center mt-3 mb-3">
      <button v-for="(pag, index) in getAllPaginationBtns" @click="paginatedData(index)"
        class="join-item btn-sm hover:bg-slate-600 disabled:bg-secondary" :disabled="index === pageNumber" :class="{
